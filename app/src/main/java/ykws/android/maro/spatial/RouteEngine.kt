 ...
 *
 * **Where a second engine would slot in.** A second engine implements this interface and is
 * constructed in place of [`ykws.android.maro.spatial.mesh.MeshRouteEngine`]; nothing else in the
 * app changes. The toggle gates on [state], [RouteViewModel]'s two callers hand [route] the three
 * answers above, and everything downstream — [`ykws.android.maro.ui.map.RoutePlan`], the trip figure,
 * the save — reads [RouteResult] and that plan, never an engine. The candidate is the taut-string
 * design of `xTrack/Route/260920_FEAT_PLN_Route_taunt_algorithm-design.md`, **parked**: no such
 * engine exists in this source tree on purpose, because an implementation that answers nothing is
 * dead code, and that design's own §13 Stage 0 is the entry point when the word comes.
 *
...
interface RouteEngine {
     ...
     * first search of a session, and it is exactly what a bundle-reading engine reaches after
     * decoding its fabric alone: the shipped mesh engine loads the mesh in [prepare] and the live
     * layers per search. An engine for which a permissive answer is not acceptable must not report
     * [RouteEngineState.Ready] until it can answer honestly — and that is a promise this interface
     * leaves to it, because only the engine knows what it reads.
     */
