 ...
 * its engine from a pace provider and a world provider** — the `CustomSortField` shape the
 * rulebook names, so no engine holds user-facing text and both locales carry the label.
 *
 ...
 * the pace provider — `() -> Double`, answering the pace in force at answer time — and the world
 * provider — `() -> AvoidWorld`, the map's live coastline world — so
 * an engine that prices at the app's pace and reads the water asks both fresh, while one that
 * ignores them stays silent. The id is the persisted value ([AppConfig.routeEngineId]'s default),
 * and [resolve] is the one lookup: it answers the shipped default for an id nothing claims, so a
 * stale stored id falls back rather than leaving the app with no engine to arm.
 */
data class RouteEngineChoice(
    val id: String,
    val labelResId: Int,
    val factory: (paceKn: () -> Double, world: () -> AvoidWorld) -> RouteEngine
) {
    companion object {
        ...
        val all: List<RouteEngineChoice> = listOf(
            RouteEngineChoice(
                ...
                factory = { _, _ -> RouteDummyEngine() }
            ),
            RouteEngineChoice(
                id = "avoid",
                labelResId = R.string.route_engine_avoid,
                factory = { paceKn, world -> RouteAvoidEngine(paceKn, world) }
            )
        )
