...
class HeatmapRampPropertiesTest {
    ...
    fun theShippedFileParsesToItsSevenFamilies() {
        ...
        assertEquals(listOf(5f, 7f, 10f, 13f, 22f, 32f, 70f), families.map { it.maxKn })
        assertEquals(
            listOf(1.0f, 0.25f, 0.5f, 0.5f, 1.0f, 1.0f, 2.0f),
            families.map { it.stepKn }
        )
        assertEquals(
            listOf(
                0xFF1A6B1A, 0xFF2AAB2A, 0xFF105189, 0xFF4EA7FF,
                0xFFFFD164, 0xFFEF6C00, 0xFF8B1515
            ).map { it.toInt() },
            families.map { it.fromArgb }
        )
        assertEquals(
            listOf(
                0xFF2AAB2A, 0xFF105189, 0xFF4EA7FF, 0xFFFFD164,
                0xFFEF6C00, 0xFF8B1515, 0xFF791FB0
            ).map { it.toInt() },
    ...
    /**
     * The code's own defaults — `AppConfig`'s seven families, its neutral tint and its five scale
     * rows — are tied to the file here: a drift between the two now fails this suite instead of
     * shipping silently. The file is the source of truth, so it is the default that follows it — the
     * three-family drift the ramp was carrying is settled by this equality rather than kept beside it.
     */
    @Test
