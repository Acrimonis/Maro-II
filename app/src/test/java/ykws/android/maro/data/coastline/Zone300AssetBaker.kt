 ...
 *
 * @Ignore by design — it needs network (Overpass). Run it whenever the bundled data must be
 * refreshed (e.g. after a classifier/builder fix), then commit the regenerated `.bin` and
 * rebuild/ship the APK:
 *
 *   ./gradlew :app:testDebugUnitTest --tests "*Zone300AssetBaker*"
 */
@Ignore("Manual online baker — requires network (Overpass); run explicitly to refresh the bundled asset.")
class Zone300AssetBaker {

    @Test
    fun `bake coastline plus 300m band into the bundled asset`() = runBlocking {
