...
REM ===================================================
REM  Maro II APK Push Script
REM  Installs a pre-built debug APK onto a connected
REM  Android device or emulator. Does NOT build.
REM  Use apk-deploy.bat for build + push.
REM ===================================================
...
REM --- Step 2: Verify APK exists (must be pre-built) ---
call :check_apk
if !ERRORLEVEL! NEQ 0 (
...
echo  [ERROR] APK not found at: !APK_RELATIVE!
echo          Run apk-build.bat first, then apk-push.bat.
echo          Or use apk-deploy.bat to build + push in one step.
echo.
exit /b 1
