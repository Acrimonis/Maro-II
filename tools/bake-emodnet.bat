...
REM ---------------------------------------------------------------------------
REM Bake the EMODnet DTM 2024 deep backbone. Output goes to D:\.src\.data\depth\
REM   (shared master copy) — apk-deploy.bat syncs to data\app-assets\depth\.
REM Clip box (W/S/E/N) + REGION come from bake-env.bat (derived from the region props).
REM The big E5 tile download is CACHED in D:\.src\.data\emodnet (shared across projects).
...
gdal_translate -of AAIGrid -co FORCE_CELLSIZE=TRUE "%WORK%\clip.tif" "%SHARED%\emodnet-%REGION%.asc" || (echo gdal_translate failed & exit /b 1)
echo Done -^> %SHARED%\emodnet-%REGION%.asc
endlocal
