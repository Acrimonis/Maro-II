...
## Focus History
- [2026-09-11 08:24 UTC] Ui_Settings — phase 1 tab finalization IMPLEMENTED + validated on device on feature/settings-menu-clean: sentence-case 18sp section titles (SectionHeader `uppercase` param removed), SettingsToggleRow → box-less ToggleRowContent (Coastline = 1-row card; Orientation aids = 1 card / 3 rows / 2 dividers), tab strip → M3 SecondaryScrollableTabRow with custom content-sized cells at 18sp SemiBold + full-cell indicator, new ui.font.tab.size token, guideline §1/§2.1/§2.3/§2.4/§2.5/§2.6/§2.9/§4 rewritten; builds SUCCESS incl. a clean --rerun-tasks build; Ask review PASS → xTrack/Ui_Settings/FEAT_HYD_Ui_Settings.md
