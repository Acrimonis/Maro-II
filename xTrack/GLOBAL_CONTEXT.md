...
- **Last Bake:** 2026-06-08 20:23 (MapDisplay — low-depth warning overlay (configurable threshold slider 0.5–5.0 m, default 1.5, persisted) above the depth raster; "Keep phone on" → top of Power saving; Mercator offset fixed by latitude-banding BOTH depth + warning (addBandedOverlay, 8 strips); land kept off ALL depth layers at the DATA level — DepthZoneMask now nulls !isWater cells, re-baked nice-frejus.bin (validation passed, datumMismatch=false) so colour map + isobaths are land-free too. assembleDebug green; DepthZoneMask + prebake-batch.md committed feature/layout-lowdepth (re-baked .bin is gitignored — regenerate via tools\bake-depth.bat). KNOWN: pink warning laps ~½ cell onto land at 25 m granularity (tracked todo: sub-cell test / vector clip). NEXT: decide bleed fix + on-device verify)
