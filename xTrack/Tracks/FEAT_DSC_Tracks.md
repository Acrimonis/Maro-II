...
- **trace-flag-and-display (2026-09-22, `feature/route-dummy`)** — a route saved from the mode is a **trace** now, and it is filtered, drawn and read as one. The flag was already the fact — `Track.plannedCourse` at proto 19 — so nothing was spent on the wire: it is **renamed `trace`** with its KDocs and handed to `TrackFromCourse`, projected into `TrackSummary` at its own proto 19 beside the land count's 18, and the index **rebuilds once** through a version stamp rather than hoping, a missing bool decoding false and every stored route otherwise reading as a recording. The lists gained a third axis, **All · Tracks · Traces**, on the shipped `FilterAxisSpec` shape with the live track exempt and the linked map filter governing the map with no per-axis code written; the trace joined the existing families rather than taking a subtree of its own (`tracking.color.traceFrom/traceTo`, `tracking.transparency.traceFrom/traceTo`, `track.width.trace`, `tracking.trace.render.nb`) and took the only two names of its own, the role's gates `tracking.trace.allowSpeedColor=false` and `allowSpeedArrows=true`; the pair is injected through a new `propColor` because the family's `propInt` cannot carry ARGB, and with it the four `tracking.color.*` keys that could never apply, their three unread `AppSettings` fields, their BuildConfig fields and their **stored preferences** all left, an unreadable value now reported by the app at start instead of falling back in silence. The map decides the **trace role before the pinned one**, so a trace takes its own pair, ladder, stroke and count whatever its pin says while a pinned trace escapes the count, and the legend reads the trace's own gate; the card shows **Dist · Total · Avg** with the two estimates labelled, its header carrying the route's **creation** instant and no end time, Resume refused through **one predicate on the summary** and merge refused as a **candidacy**, and "Follow again" recorded as a suggestion and deliberately left out. `:app:compileDebugKotlin`, `apk-build.bat` and `:app:testDebugUnitTest` green at **580 tests**; the run's review held R29–R42 with **no breach** and its seven findings were closed — the trace-blind legend that this change itself created, the app-side report for an unreadable colour, the count and the pinned escape, the pinned-as-unpinned tautology, the list's accent strip, three prose drifts and two test gaps — each new guard shown **red on the revert it catches**. The device pass the plan names stays owed: a route's look beside a recording, the card's three cells, the filter's three values and the count's effect → `xTrack/Tracks/260922_FEAT_PLN_Tracks_trace-flag-and-display.md`
