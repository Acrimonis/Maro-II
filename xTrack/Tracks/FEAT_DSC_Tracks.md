...
- [ ] 6 · Scale visibility — show it whenever a ramp-painted stroke is on the map: Colours with stored tracks drawn **and the tracks layer on**, or the eye banding a selection, so unselecting in Colours keeps it; the predicate and its six cases are in the outlines plan §2.2
- [ ] 7 · Scale rendering — the strip takes the top-left control row's two edges, from a width the row's own composition must first supply, and its background takes a token of its own instead of the text-scrim family
