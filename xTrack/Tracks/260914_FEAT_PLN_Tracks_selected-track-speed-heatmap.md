...
- **Walk closed 2026-09-14, the device E2E dropped:** the walk's item 14 was closed as dropped rather than deferred — the run is not that session's work — which exhausts the level and moves its open points here: the E2E stays unrun, and the five residues named in the bullet above become this plan's open list rather than the walk's. The render-mode generalization that followed is walked in a new level, over a plan of its own.
