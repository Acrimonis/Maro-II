...
## Implemented
- **Stage 0 — feasibility harness (2026-09-20)** — the corridor, the standoff-dilated exclusion set, the tangent graph, the turn-aware A* and the speed profile now run as JVM code over the real `nice-menton` assets, with 30 tests green through `gradlew :app:testDebugUnitTest --tests "ykws.android.maro.route.*"` and no change to the APK, an asset or the dependency list — which is what makes D6 answerable by measurement. The synthetic wall-with-a-gap case passes with its known-by-hand detour, every hard constraint validated on the emitted track, two runs byte-identical and A* equal to the reference Dijkstra to 1e-6 once corners are priced at nothing; the acceptance pair derived from the repo's own track files is refused under **both** D3 variants — with the hard standoff the start sits 7 m from the coast and inside the 25 m margin, and with the priced standoff the graph finds no visible chain at 838 vertices and 1 868 edges — so the real endpoint pair is still to be pinned. Review findings open: the neighbour prune recomputes about ten million haversines per attempt (39 s measured at 838 vertices), zone holes were made visibility barriers against §10.2's "navigable, geometry, not an obstacle", and §13.3's priced-versus-naive zone-distance assertion is absent → `260920_FEAT_PLN_Route_algorithm-design.md`
