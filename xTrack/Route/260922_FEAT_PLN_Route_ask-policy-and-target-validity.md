...
| R16 | **The draft's outcomes**: **Route** · **Save track and Route** · **Save track and Exit** · **Exit**, with one pin checkbox governing the saves (names revised 2026-09-23 with the panel's anatomy — the fourth was `Cancel`, and one word now carries the door in both phases) | decided |
| R17 | **The following panel's actions**: `Freeze/Resume` · `Save track` · `Exit`, with **Abort** replacing the first while a refresh runs — the save takes the accent and leaving is outlined (roles revised 2026-09-23, the panel alignment) | decided |
...
| R23 | **Leaving the following phase asks first, by either door** — the toggle's exit **and** the panel's own **Exit** raise the same **Save track and Exit** · **Continue** · **Discard route** dialog (words and order revised 2026-09-23: the affirmative first, the neutral stay, the loss last), its save carrying the last/all scope, because that Exit can save the front route only; **leaving the draft asks nothing**; leaving **cancels the call in flight**, and a failure arriving afterwards is not toasted | decided |
