...
**Last Bake:** 2026-09-19 22:31 UTC — written by `#bake`; absence means never baked
...
**In design — nothing implemented, and no source file touched.** The epic went from the retired subfeature shape to a settled design in one session: registered in the routing map and the summary table, normalized to sections, its file-level todos folded in, a `route-saving` section added, and its rules rewritten so every decision is contract rather than conversation. The plan was consolidated the same day into a register at `xTrack/Route/260919_FEAT_PLN_Route_registration-and-drift.md`, which is where the why lives; the epic carries the what.
...
- `xTrack/Route/260919_FEAT_PLN_Route_registration-and-drift.md` — the consolidated design and its register of decisions
- `xTrack/GLOBAL_CONTEXT.md` — Route's routing row, its summary row and the focus entry
- Nothing under `app/` — no source, build or asset file was changed in this session
