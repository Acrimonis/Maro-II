...
- [ ] **Build-time band bake + bundle — PLUMBING DONE 2026-06-06, run the baker to produce the asset.** Implemented: (a) `Zone300AssetBaker` (@Ignore JUnit, **online**) — fetches OSM, builds the band with the same containment `isWater` + cell size as prod, serializes via `CoastlineSerializer`, writes `app/src/main/assets/coastlines/nice-frejus.bin`; (b) `CoastlineRepository.loadCoastline` now prefers **disk cache → bundled asset → live fetch** (`readFromAsset` + `appContext` from `setCacheDir`), so a shipped app loads the prebaked band with no on-device fetch/regen. **TO SHIP:** run `:app:testDebugUnitTest --tests "*Zone300AssetBaker*"` on a networked machine → commit the generated `.bin` → `apk-build.bat` → ship. Could NOT run the baker here (no network). Optional: wire it into a Gradle task for auto-rebake (keeps offline builds working only if the `.bin` is committed).
