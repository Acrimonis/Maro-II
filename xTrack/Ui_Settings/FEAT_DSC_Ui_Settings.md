...
- **C12 OverlayLayer param collapse — Tiers 1a-i, 1a-ii, 1b (2026-09-10, `feature/refact-C12`)** — first three slices of the MapScreen step-2 refactor's C12. New same-package `OverlayLayerParams.kt` holds four public `@Immutable` data classes: `OverlayChrome` (7 — drawer/wizard visibility flags plus `wizardStep` and `drawerState`), `MenuOverlayData` (12 — GPS mode, menu toggles, first-track/marker ids, track/marker map-referential filters and counts), `SettingsOverlayData` (5 — settings tab plus its four scroll positions) and `TrackInfoOverlayData` (4 — track-info drawer visibility, track, id list, index). `OverlayLayer` dropped **89 → 65 params**, removing the read-only params from several separated signature regions while leaving every interleaved callback in place, and unpacks all four bundles into 28 same-named locals at the top of the body so the body and every child call are unchanged; the single call site in `MapScreen.kt` builds the bundles inline (plain values, never `remember`-ed). Public visibility is required — a public `OverlayLayer` cannot expose an `internal` type. `apk-build.bat` SUCCESS with zero new warnings on all three tiers; Ask reviews 8/8 and 8/8 pass, and Tier 1b was verified by direct pre-image `git diff` (9 params out, 2 in; the four `ScrollState` fields proven paired display/navigation/position/system on all three legs; `trackListIds`/`currentTrackIndex` expressions byte-identical). The R10 recomposition metric is waived in writing (plan R10 entry). Tiers 1a-i+1a-ii committed as `ec57458`; Tier 1b uncommitted at time of writing. Remaining C12: Tier 1c `TrackListOverlayData` (3) + `MarkerListOverlayData` (4) plus the 2 dead `rememberLazyListState` defaults, then the citation-drift pass → `xTrack/Ui_Settings/260909_FEAT_PLN_Ui_Settings_mapScreen-orchestration-monolith-refactor.md` (C12 APPENDIX)
