...
| R11 | `#new` left the branch tracking `origin/develop` (git: "set up to track remote branch 'develop'" — the shape that command produced until 2026-09-21, when it began cutting with `--no-track` and `#push` took on writing the branch's own upstream), so a bare `git push` under `push.default=upstream` would target the one forbidden branch | Med | `#push` is safe (explicit refspec); before any bare push run `git push -u origin feature/refact-C12` once, or `git branch --unset-upstream` |
...
- **Known nit — resolved in Tier 1b (2026-09-10):** the destructure-block comment now reads "Destructured bundle locals (chrome, menu, settings, track info)".
- **Tier 1b result (2026-09-10):** landed — `SettingsOverlayData` (5) + `TrackInfoOverlayData` (4) added; `OverlayLayer` 72 → 65 params; destructure 19 → 28 lines; `apk-build.bat` SUCCESS with zero new warnings. Verified by a direct `git diff` against the pre-image: exactly 9 params removed and 2 added, each destructured local name-for-name identical to the removed param, and the 9 call-site expressions reproduced byte-for-byte — including `trackListIds` / `currentTrackIndex`, whose long `trackSummaries.filter { … }.map { … }` expressions appear character-identical on both the removed and the added side. The four `ScrollState` fields are paired display→display, navigation→navigation, position→position, system→system on all three legs, so the cross-wire hazard is disproven by evidence rather than by inspection.
- **Ask-stage deviation for Tier 1b (2026-09-10):** the Ask review hop was skipped for this tier only. Rationale: the tier's single material risk was the silent `ScrollState` cross-wire, and the direct pre-image `git diff` above is strictly stronger evidence than a read-only review (Ask has no shell, so it cannot obtain the pre-image — as it noted in its own Tier 1a-ii limitation). Re-run `#implement`'s Ask hop on the tier if an independent code-health opinion is wanted before merge.
