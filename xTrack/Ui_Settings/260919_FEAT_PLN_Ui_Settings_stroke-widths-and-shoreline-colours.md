...
- Its upstream is bound to `origin/develop` itself — the shape `#new`'s `checkout -b` produced until 2026-09-21, when the command began cutting with `--no-track` and `#push` took on writing the branch's own upstream — so a bare push from it refuses rather than publishing, under the `simple` default measured on 2026-09-21.
