...
- **Census derived 2026-09-17, re-derived after the reply-shape change — one statement replacing the rest.** A search over the rule bullets returns **4 🛑 · 6 ⛔ · 5 💬 · 4 🧹 · 1 🟢 = 20 rules**, the fifth 💬 being `TECHNICAL CHOICE IS THE AGENT'S` and the fourth 🛑 the dependency-approval clause an earlier review found unmarked in §4. Four earlier counts were kept in prose — four 🛑 before the git pair merged, then eighteen, then nineteen, then twenty — which is why anything needing this number should re-run that search rather than quote this line.
...
**Shipped 2026-09-17 on `feature/some-wflw`** — the five-tier legend with every rule re-marked; `SCOPE LOCK` reworded; `NEVER ASSUME` and `Challenge` consolidated into `ASK, DON'T GUESS`; `MODE LOCK` reframed on acting with its permitted close; the git rule named and compounded as `GIT WRITES AND DEPLOYS ARE THE USER'S CALL` with its read-only principle; `PROTECTED BRANCHES` written with its mechanical refusal; `NO BINARY READS` restated as a class; `DEVICE EVIDENCE ON REQUEST` renamed and converted from an ask into a status; `#rule` rebuilt as legend, tier reload and `all` staleness verdict; `#doctor` check (s) added; the one-sentence `Directive trace:` carried into the hydration by `#bake`; and the `#review` sweep landed over the five covered classes. Deviations: the registry of ids was declined (D-B), the trim remains a separate task with its candidate list at §3.6 (D-E), and two residues stay open in the feature's `## Todos` — the `ASK, DON'T GUESS` first-clause rescope and the ⛔ two-force reading.
