...
Nothing else is subtracted. `Recommendations argue against themselves`, the question threshold, the
gate rules and every Core Directive stay in force in brief mode — as does `Answer only what was asked,
then stop`, which a shorter reply obeys more easily, not less.
